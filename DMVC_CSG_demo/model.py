...
from metric import valid, visualize_data_tsne, valid2
import time
...
def contrastive_train(model, mv_data, mvc_loss, batch_size, epoch, lmd, beta, optimizer, k):
    model.train()
    ...
    for batch_idx, (sub_data_views, _, sample_idx) in enumerate(mv_data_loader):
        ...
        loss_list = list()
        loss_list.append(beta * mvc_loss.feature_loss(q_centers))
        for i in range(num_views):
            k_centers = model.compute_centers(zs[i], batch_psedo_label)
            loss_list.append(criterion(sub_data_views[i], xrs[i]))
            loss_list.append(beta * mvc_loss.feature_loss2(q_centers, k_centers))         #pro
            loss_list.append(
                lmd*mvc_loss.compute_cluster_loss(num_clusters, common_z, q_centers, batch_psedo_label, zs[i], k))
        loss = sum(loss_list)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
